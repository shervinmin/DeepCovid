...
parser.add_argument('--trained_model_path', type=str, default='./models/covid_resnet18_epoch100.pt',
                      help='The path and name of trained model')

parser.add_argument('--cut_off_threshold', type=int, default= 0.2, 
...
def find_sens_spec( covid_prob, noncovid_prob, thresh):
    sensitivity= (covid_prob >= thresh).sum()   / len(covid_prob)
    specificity= (noncovid_prob < thresh).sum() / len(noncovid_prob)
...
plt.show()
plt.savefig('./confusion_matrix.png', bbox_inches='tight', pad_inches=0) #dpi = 200
...
plt.legend(loc='upper center')
plt.show()
plt.savefig('./scores_histogram.png', bbox_inches='tight', pad_inches=0) #dpi = 200
...
model_name= 'resnet18' ## resnet18, resnet50, Squeezenet, densenet161
covid_prob_res18   = pickle.load( open("./results/covid_prob_%s.p" %model_name,"rb") )
non_prob_9_res18   = pickle.load( open("./results/non_prob_9_%s.p" %model_name,"rb") )
non_prob_all_res18 = pickle.load( open("./results/non_prob_all_%s.p" %model_name,"rb") )
y_test_res18= [1 for i in range(len(covid_prob_res18))]+[0 for i in range(len(non_prob_9_res18))]
y_pred_res18= [covid_prob_res18[i] for i in range(len(covid_prob_res18))]+[non_prob_9_res18[i] for i in range(len(non_prob_9_res18))]
...
plt.savefig('./ROC_covid19.png', bbox_inches='tight', pad_inches=0) #dpi = 200
